...
function scn_display_course_details_mb( $scn_course ) { 
    ...
    $school_year = get_post_meta( $scn_course->ID, 'school_year', true);

    $all_teachers = array( 'Master A', 'Master B', 'Dr C' );
	?>
	<table>
        <tr>
			<td style="width: 150px">School Year</td>
			<td><input type="text" style="width:100%" name="scn_course_school_year" value="<?php echo intval( $school_year ); ?>"/></td>
		</tr>
		<tr>
...
function scn_add_course_fields( $course_id, $course ) {
	...
	if ( isset( $_POST['scn_course_school_year'] ) ) {
		update_post_meta( $course_id, 'school_year', sanitize_text_field( $_POST['scn_course_school_year'] ) );
	}
...
add_filter( 'manage_edit-scn_course_columns', 'scn_course_add_columns' );

// Function to add columns for author and type in book review listing
// and remove comments columns
function scn_course_add_columns( $columns ) {
    $new_columns = array();
    $new_columns['scn_course_school_year'] = 'School Year';
...
add_action( 'manage_posts_custom_column', 'scn_course_populate_columns' );

// Function to send data for custom columns when displaying items
function scn_course_populate_columns( $column ) {
	if ( 'scn_course' != get_post_type() ) {
		return;
    }

	// Check column name and send back appropriate data
	if ( 'scn_course_school_year' == $column ) {
        echo esc_html( get_post_meta( get_the_ID(), 'school_year', true ) );
    }
    elseif ( 'scn_course_teacher' == $column ) {
...
function scn_course_column_sortable( $columns ) {
	$columns['scn_course_school_year'] = 'scn_course_school_year';
